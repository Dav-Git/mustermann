    ...
    <div class="container py-4">
      ...
      <div class="p-4 mb-4 mt-4 bg-white rounded-3 border border-ligh">
        <div class="container-fluid py-5">
          ...
          <p>Sind sie zufrieden mit unserem Service? Nein? Das ist doof. Bitte kommen Sie wieder.</p>
          <?php
          ...
          ?>
            <form action="feedback.php" method="POST">
              <div class="form-group">
                <input type="radio" name=feedback value="1" id="1" required>
                <label for="1">1 (Sehr gut)</label>
                <input type="radio" name=feedback value="2" id="2">
                <label for="2">2</label>
                <input type="radio" name=feedback value="3" id="3">
                <label for="3">3</label>
                <input type="radio" name=feedback value="4" id="4">
                <label for="4">4</label>
                <input type="radio" name=feedback value="5" id="5">
                <label for="5">5 (Sehr schlecht)</label>

                <label for="name">Ihr Name</label>
                <input type="text" class="form-control" name="name" id="name" placeholder="Name" required>
                <label for="email">Ihre E-Mail-Adresse</label>
                <input type="email" class="form-control" name="email" id="email" placeholder="E-Mail" required>
                <label for="kommentar">Ihr Feedback</label>
                <textarea class="form-control" name="kommentar" id="kommentar" rows="3" placeholder="Ihr Feedback" required></textarea>
              </div>
          ...
          if (isset($_POST['feedback'])) {
            ...
            mysqli_query($link, "INSERT INTO feedback (feedback, name, email, note, datum) VALUES ('" . htmlspecialchars($_POST['kommentar']) . "', '" . htmlspecialchars($_POST['name']) . "', '" . htmlspecialchars($_POST['email']) . "', '" . htmlspecialchars($_POST['feedback']) . "', '" . date('Y-m-d H:i:s') . "')");
          }
          ?>
