        ...
        <div class="container py-4">
            ...
            <div class="p-4 mb-4 mt-4 bg-white rounded-3 border border-ligh">
                <div class="container-fluid py-5">
                    ...
                    <h1 class="display-5 fw-bold">Support-Team</h1>
                    <?php
                    $link = mysqli_connect("localhost", "root", "", "mustermann");
                    ?>


                    <strong>Anzahl Teammitglieder:</strong> <?php echo mysqli_fetch_row(mysqli_query($link, "SELECT count(*) FROM team"))[0] ?><br>
                    <strong>Support-Team (alphabetisch):</strong>
                    <?php
                    $result = mysqli_query($link, "SELECT nachname FROM team ORDER BY nachname ASC");
                    while ($row = mysqli_fetch_row($result)) {
                        echo $row[0] . ", ";
                    } ?><br>
                    <strong>Support-Team (langjährige Mitarbeiter:innen):</strong>
                    <?php
                    $result = mysqli_query($link, "SELECT nachname FROM team WHERE eintrittsjahr < 2018 ORDER BY nachname ASC");
                    while ($row = mysqli_fetch_row($result)) {
                        echo $row[0] . ", ";
                    } ?><br>
                    <strong>Support-Team (nach Firmeneintritt):</strong>
                    <?php
                    $result = mysqli_query($link, "SELECT nachname,eintrittsjahr FROM team ORDER BY eintrittsjahr DESC");
                    while ($row = mysqli_fetch_row($result)) {
                        echo $row[0] . " (" . $row[1] . "), ";
                    } ?><br><br>
                    <strong>Support-Team in Liste:</strong>
                    <ul>
                        <?php
                        $result = mysqli_query($link, "SELECT vorname,nachname FROM team ORDER BY eintrittsjahr DESC");
                        while ($row = mysqli_fetch_row($result)) {
